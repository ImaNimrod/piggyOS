...
void tree_node_destroy(tree_node_t * node) {
	/* kfree the contents of a node and its children, but not the nodes themselves */
...
void tree_destroy(tree_t * tree) {
	/* kfree the contents of a tree, but not the nodes */
	if (tree->root) {
		tree_node_destroy(tree->root);
	}
}

void tree_node_kfree(tree_node_t * node) {
	/* kfree a node and its children, but not their contents */
	if (!node) return;
	foreach(child, node->children) {
		tree_node_kfree(child->value);
	}
	kfree(node);
}

void tree_kfree(tree_t * tree) {
	/* kfree all of the nodes in a tree, but not their contents */
	tree_node_kfree(tree->root);
}
